...
pub fn is_valid_palindrome(input_string: &str) -> bool {
	...
	if last_index <= first_index {
	    return true;
             ...
             if input_string_vec[first_index].to_uppercase().collect::<String>() != input_string_vec[last_index].to_uppercase().collect::<String>() {
             	return false;
       	     }
...
mod tests {
    ...
    pub fn test_valid_palindrome_selim() {
     	let result = Solution::is_palindrome(String::from("\"Sue,\" Tom smiles, \"Selim smote us.\""));
	assert_eq!(result, true);
    }
